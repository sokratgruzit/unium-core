...
const password2Route = withLogin2(
  pickMethod({
    DELETE: request => {
      const { login } = request
...
const recovery2Route = withLogin2(
  pickMethod({
    DELETE: request => {
      const { login } = request
...
const messagesRoute = pickMethod({
  POST: request => {
    const { db, json } = request
    const { loginIds } = json
...
const server = pickServer(urls, request =>
 ...
 */
export function makeFakeServer(db) {
